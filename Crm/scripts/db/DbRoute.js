dbTools.routeListGet = function(prdBgn, datasetGet) {
    ...
    dbTools.db.transaction(function(tx) {
        var sql = "SELECT VP.visitPlanId, VPI.visitPlanItemId, VP.empId, VP.dateBgn, VPI.custId, K.name, K.cityId, C.name AS cityName, K.addr, K.chainId, CN.name AS chainName, NULL AS docId, 1 AS docType"
            + " FROM VisitPlan VP"
...
dbTools.routeListDocGet = function(visitPlanItemId, datasetGet) {
    ...
    dbTools.db.transaction(function(tx) {
        var sql = "SELECT VPI.visitPlanId, VP.dateBgn, VPI.visitPlanItemId, VPI.custId, K.name AS custName"
...
dbTools.routeListDocItemListGet = function(visitPlanItemId, datasetGet) {
    log("routeListItemGet");
    dbTools.db.transaction(function(tx) {
        var sql = "SELECT S.skuId, S.name, S.brandGrpId, BG.name AS brandGrpName, BG.brandId, B.name AS brandName, S.skuCatId, SC.name AS skuCatName, SC.parentId AS skuCatParentId, NULL AS qnt"
            + " FROM Sku S"
